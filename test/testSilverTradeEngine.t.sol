...
contract TestTradeEngine is Test {
    ...
    modifier addSilverStockInStoreA() {
        ...
        for (uint256 i = 0; i < 5; i++) {
            vault.registerBar(randomSilverIdOne[i], _weight[0], _purity, redeemLocationA);
            vault.registerBar(randomSilverIdTwo[i], _weight[1], _purity, redeemLocationA);
            vault.registerBar(randomSilverIdThree[i], _weight[2], _purity, redeemLocationA);
            vault.registerBar(randomSilverIdFour[i], _weight[3], _purity, redeemLocationA);
        }
        vm.stopPrank();
        _;
    }
    ...
    function testBuySilver() public {
        console.log(stableCoin.balanceOf(BUYER));
    ...
    function testBuySilverERC20() public{
