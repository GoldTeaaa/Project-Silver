...
SIXTEEN_SILVER_PRICE := 1980000000000000000000
...
grantBurnTradeEngine :; cast send $(SILVER_ERC20_ADDRESS) "grantRole(bytes32,address)" $(BURNER_ROLE) "$(SILVER_ENGINE_ADDRESS)" --private-key $(ANVIL_PRIVATE_KEY_1) --rpc-url $(ANVIL_RPC_URL)
...
oraclePrice :; cast call $(ORACLE_ADDRESS) "latestRoundData()" --rpc-url $(ANVIL_RPC_URL)

directMint :; cast send $(SILVER_ERC20_ADDRESS) "mint(address,uint256)" $(ANVIL_ADDRESS_2) $(AMOUNT) --private-key $(ANVIL_PRIVATE_KEY_1) --rpc-url $(ANVIL_RPC_URL)

totalSupply :; cast call $(STABLECOIN_ADDRESS) "totalSupply()" --rpc-url $(ANVIL_RPC_URL)